...
# upload part size, unit is MB. smaller file would use put_object method, bigger file would use upload_part method.
_buff_size = 5
...
class OssSftpHandle(paramiko.SFTPHandle):
    def __init__(self, bucket, key, flags):
        ...
        if self.flags == os.O_RDONLY:
            self.obj = self.bucket.get_object(key)
...
class OssSftpServerInterface(paramiko.SFTPServerInterface):
    ...
    def list_folder(self, path):
        out = []
        prefix = self._convert_to_oss_path(path)
    ...
    def stat(self, path):
        oss_path = self._convert_to_oss_path(path)
    ...
    def open(self, path, flags, attr):
        object_name = self._convert_to_oss_path(path)
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--buff_size", dest="buff_size", type='int', default='5', help="the buff size is used for send data, default is 5, unit is MB.")
