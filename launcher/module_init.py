...
def start(module):
    ...
    try:
        ...
        if module == 'ossftp':
            ...
            passive_ports = config.get(["modules", "ossftp", "passive_ports"], "51000~52000")
            buff_size = config.get(["modules", "ossftp", "buff_size"], 5)
            ...
            if not os.path.isfile(script_path):
                ...
                return "fail"
            cmd = [sys.executable, script_path, "--listen_address=%s" % address, "--port=%d" % port,
                   "--loglevel=%s" % log_level, "--bucket_endpoints=%s" % bucket_endpoints,
                   "--passive_ports=%s" % passive_ports, "--buff_size=%d" % buff_size, "--protocol=%s" % protocol]
            print(cmd)
            ...
            buff_size = config.get(["modules", "osssftp", "buff_size"], 5)
            protocol = config.get(["modules", "launcher", "oss_protocol"], 'https')
