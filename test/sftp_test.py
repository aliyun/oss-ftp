...
class SftpTest2(unittest.TestCase):
    def setUp(self):
        ...
        self.key_id = get_value_from_config("sftpconfig", "normal_id")
        self.bucket_name = get_value_from_config("sftpconfig", "normal_bucket")
        self.username = self.key_id + '/' + self.bucket_name
        self.password = get_value_from_config("sftpconfig", "normal_key")
    ...
    def test_sftp_login_error(self):
        ...
        tran = paramiko.Transport(s)
        try:
            tran.connect(username=self.key_id+'-error' + '/' + self.bucket_name, password=self.password + '-error')
        except AuthenticationException as e:
            pass
        finally:
            tran.close()
