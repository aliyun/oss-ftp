...
class Http_Handler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def passive_ports_check(self, passive_ports_str):
        pattern = r'^\d+~\d+$'
        if re.match(pattern, passive_ports_str):
            try:
                ports_str_list = passive_ports_str.split("~")
    ...
    def req_config_handler(self):
        ...
        if reqs['cmd'] == ['get_config']:
            config.load()
            data = '{ "popup_webui": %d, "show_systray": %d, "auto_start": %d, "oss_protocol": "%s", ' \
                   '"ossftp_address": "%s", "ossftp_port": %d, "ossftp_loglevel": "%s", "ossftp_bucketendpoints": "%s", "ossftp_enable":%d, ' \
                   '"ossftp_passive_ports": "%s", "ossftp_buff_size": %d, ' \
                   ...
                   (config.get(["modules", "launcher", "popup_webui"], 1)
                    ...
                    , config.get(["modules", "ossftp", "passive_ports"], '')
            ...
            ossftp_passive_ports = config.get(["modules", "ossftp", "passive_ports"], '51000~52000')
            ...
            if success and 'ossftp_passive_ports' in reqs:
                ossftp_passive_ports = reqs['ossftp_passive_ports'][0].strip()
                if not self.passive_ports_check(ossftp_passive_ports):
                    success = False
                    data = '{"res":"fail, ossftp_passive_ports:%s illegal, should be like 51000~520000"}' % ossftp_passive_ports
            ...
            if success:
                ...
                config.set(["modules", "ossftp", "passive_ports"], ossftp_passive_ports)
