...
- **免安装:** 解压后可直接运行
- **免设置:** 无需设置即可运行
- **透明化:** F工具是python写的，您可以看到完整的源码，我们已开源到[Github](https://github.com/aliyun/oss-ftp)

### 主要功能

- 支持文件／文件夹的上传／下载／删除等操作
- 通过Multipart方式，分片上传大文件
- 支持大部分FTP/SFTP指令，可以满足日常FTP/SFTP的使用需求

### *注意*
- 目前在1.0版本中，考虑到安装部署的简便, OSS FTP工具没有支持TLS加密。由于FTP协议是明文传输的，**为了防止您的密码泄漏，建议将FTP server和client运行在同一台机器上**，通过127.0.0.1:port的方式来访问
- 不支持rename, move操作
- 安装包解压后的路径不要含有中文
- FTP/SFTP server的管理控制页面在低版本的IE中可能打不开
- FTP/SFTP server支持的Python版本: Linux与macOS支持python2.6和 python2.7，Windows支持python3.6


## 启动FTP/SFTP Server
分为便捷式启动与轻量级启动两种方式。

### 1. 便捷式启动带WEB配置的FTP/SFTP server
启动FTP与SFTP server的同时，启动一个简单的web server，方便进行FTP/SFTP服务器的配置和状态监控。
可以在配置界面配置FTP server与SFTP server的开启与否。

在OSS-FTP工具的根目录，直接运行对应的start脚本
...
```
passive_port_start 为FTP 被动模式的数据端口的起始端口。
passive_port_end 为FTP 被动模式的数据端口的结束端口。
...
- 登录类型： 正常
- 用户：
> - **使用Ram账号登陆**: access_key_id/bucket_name (注意： 这里的/是必须的，不是‘或’的意思，如用户名'tSxyiUM3NKswPMEp/test-hz-jh-002')
> - **使用自定义账号登陆**: 填写在config.json文件中配置的用户名
- 密码：
> - **使用Ram账号登陆**: access_key_secret
> - **使用自定义账号登陆**:填写在config.json文件中配置的用户密码
- 端口：FTP server默认监听端口：2048， SFTP server默认监听端口50000

## 配置文件config.json介绍
如果程序使用便捷式启动模式，则FTP或者SFTP server将将从config.json中获取配置信息，如果使用轻量级启动模式启动FTP/SFTP server，则config.json中仅有自定义用户accounts配置生效。
...
```
- access_id: 阿里云Ram账号的access_id
- access_secret: 阿里云Ram账号的access_key
- bucket_name: 需要登陆的bucket名称
- home_dir: 根目录，比如"test/"表示根目录为"test"目录。
- login_password: 自定义用户的登陆密码
- login_username: 自定义用户的登陆账号

### 启动参数
- auto_start: 是否开机自启，默认值0表示不自启。
- control_port: web配置端口，默认8192。
- language: 语言设置，cn表示中文，en表示英文。
...
- passive_port_start: FTP server 被动模式数据传输端口范围的起始端口，默认值51000。
- passive_port_end FTP server 被动模式数据传输端口范围的结束端口，默认值53000。
...
## Run tests
进入 *test*目录, 配置test.cfg然后运行run_test.sh脚本
```bash
bash run_test.sh
```
