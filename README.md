...
- Python versions supported by the OSS-FTP tool: Linux and MacOS support python2.6 and Python2.7, Windows support Python3.6.
...
```bash
python ftpserver.py --listen_address=<ip address> --port=<your local port> --passive_ports_start=<the start port of passive ports> --passive_ports_start=<the end port of passive ports> --internal=<True/False> --bucket_endpoints=<format of bucket_name.enpoint> --loglevel=<DEBUG/INFO> --buff_size=<buffer size> --protocol=<https/http>
```
passive_port_start is the start of the FTP server passive ports range, default value is 51000.
passive_port_end is the end of the FTP server passive ports range, default value is 53000.
...
```
This will start a SFTP server at the 127.0.0.1:50000 port. 

You can also specify the listening address and port among other information in parameters at the startup.
```bash
python sftpserver.py --listen_address=<ip address> --port=<your local port> --internal=<True/False> --bucket_endpoints=<format of bucket_name.enpoint> --loglevel=<DEBUG/INFO> --buff_size=<buffer size> --protocol=<https/http>
```
...
We recommend you use [FileZilla Client]((https://filezilla-project.org/)) to connect to the FTP or SFTP server. After downloading and installing the tool, you can connect it to the server following the instructions below: 
...
- User:
> - **Use RAM sub-account**: access_key_id/bucket_name (Note: Here the "/" is necessary. It does not mean "or". For example, the user name 'tSxyiUM3NKswPMEp/test-hz-jh-002'). 
> - **Use customized account**: Use the login_username configuration in config.json
- 密码：
> - **Using RAM sub-account**: access_key_secret
> - **Using customized account**:Use the login_password configuration in config.json
- port: the FTP default port is 2048, the SFTP default port is 50000.

## Introduction of config.json
If you start this tools in easy startup mode, The FTP or SFTP server will get all configurations from config.json. Otherwise, your FTP or SFTP server starts in the lightweight startup mode will only use the customized account configuration in config.json.
...
```
- access_id: The access id of Ram sub-account.
- access_secret: The access key of Ram sub-account.
- bucket_name: The bucket name you access.
- home_dir: root directory，such as "test/".
- login_password: the login password.
- login_username: the login username.
...
- language: cn means Chinese, and en means English.
- oss_protocol: the transport protocol of OSS, the value is https or http,  default is https.
...
- passive_port_start: the start of the FTP server passive ports range, default value is 51000.
- passive_port_end: the end of the FTP server passive ports range, default value is 53000.
...
- buff_size: the size of upload buffer, the FTP server will pop up data and send to OSS while the buffer is full or the file is upload over. 
...
## Run tests
Enter the *test* directory, and configure the file test.cfg then run tests.
```bash
bash run_test.sh
```
